...
void Config::loadFromEnv() {
    ...
    if (const char* val = std::getenv("NEXUS_MAX_INBOUND_QUEUES")) {
        node.max_inbound_queues = clamp(static_cast<size_t>(std::atoi(val)), size_t(8), size_t(64));
    }

    if (const char* val = std::getenv("NEXUS_QUEUE_CAPACITY")) {
        node.queue_capacity = clamp(static_cast<size_t>(std::atoi(val)), size_t(64), size_t(1024));
    }

    if (const char* val = std::getenv("NEXUS_NUM_THREADS")) {
        node.num_processing_threads = clamp(static_cast<size_t>(std::atoi(val)), size_t(1), size_t(16));
    }
    ...
    if (const char* val = std::getenv("NEXUS_SHM_QUEUE_CAPACITY")) {
        shm.queue_capacity = clamp(static_cast<size_t>(std::atoi(val)), size_t(64), size_t(1024));
    }
...
size_t Config::calculateMemoryFootprint() const {
    ...
    size_t shm_memory = node.max_inbound_queues * shm.queue_capacity * shm.message_size;

    // Large data channel footprint
    size_t large_data_memory = large_data.buffer_size;

    // Async queue footprint (rough estimate)
    size_t async_memory = node.num_processing_threads * node.max_queue_size * 256;  // Estimated message size
