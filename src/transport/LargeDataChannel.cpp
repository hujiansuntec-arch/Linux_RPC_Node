...
std::shared_ptr<LargeDataChannel> LargeDataChannel::create(
    ...
    if (!g_cleanup_done.load(std::memory_order_acquire)) {
        ...
        if (!g_cleanup_done.load(std::memory_order_relaxed)) {
            ...
            if (cleaned > 0) {
                NEXUS_DEBUG("LargeData") << "Startup cleanup: removed " << cleaned 
                          << " orphaned channel(s)";
            } else {
...
LargeDataChannel::~LargeDataChannel() {
    ...
    if (control_ && control_->ref_count.load(std::memory_order_acquire) > 0) {
        ...
        NEXUS_DEBUG("LargeData") << "Destructor: " << shm_name_ 
                  << ", ref_count: " << prev_count << " -> " << (prev_count - 1);
        
        // 如果是最后一个引用，清理共享内存
        if (prev_count == 1) {
            NEXUS_DEBUG("LargeData") << "Last reference, unlinking shared memory: " 
                      << shm_name_;
            ...
            if (shm_unlink(shm_name_.c_str()) == 0) {
                ...
                NEXUS_ERROR("LargeData") << "Failed to unlink: " << shm_name_ 
                          << " (errno: " << errno << ")";
            }
...
bool LargeDataChannel::initialize() {
    ...
    if (is_creator) {
        ...
        NEXUS_DEBUG("LargeData") << "Created channel: " << shm_name_ 
        ...
        NEXUS_DEBUG("LargeData") << "Connected to channel: " << shm_name_ 
                  << ", ref_count: " << new_count
                  << ", PID: " << getpid()
                 ;
    }
...
int64_t LargeDataChannel::write(const std::string& topic, 
    ...
    if (size > config_.max_block_size) {
        NEXUS_ERROR("LargeData") << "Data size " << size << " exceeds max block size " 
        ...
        switch (config_.overflow_policy) {
            ...
            case LargeDataOverflowPolicy::DROP_NEWEST:
                // 丢弃当前写入
                {
                    ...
                    NEXUS_ERROR("LargeData") << "Buffer full, dropping newest data (total: " 
                              << dropped << ")";
...
LargeDataChannel::ReadResult LargeDataChannel::validateBlock(
    ...
    if (header->size > config_.max_block_size) {
        NEXUS_ERROR("LargeData") << "Size exceeded: " << header->size 
                  << " > " << config_.max_block_size;
    ...
    if (calculated_crc != header->crc32) {
        NEXUS_ERROR("LargeData") << "CRC32 mismatch: expected " << header->crc32 
                  << ", got " << calculated_crc;
...
void LargeDataChannel::setOverflowCallback(LargeDataOverflowCallback callback) {
    ...
    NEXUS_DEBUG("LargeData") << "Overflow callback " 
              << (callback ? "enabled" : "disabled");
}

// 清理过期的大数据通道（静态函数）
size_t LargeDataChannel::cleanupOrphanedChannels(uint32_t timeout_seconds) {
    NEXUS_DEBUG("LargeData") << "Scanning for orphaned channels (timeout: " 
              << timeout_seconds << "s)...";
    ...
    while ((entry = readdir(dir)) != nullptr) {
        ...
        if (should_cleanup) {
            if (shm_unlink(name.c_str()) == 0) {
                ...
                NEXUS_DEBUG("LargeData") << "✓ Cleaned: " << name 
                          << " (" << (shm_size / 1024 / 1024) << " MB) - " 
                          << cleanup_reason;
            } else {
                NEXUS_ERROR("LargeData") << "✗ Failed to unlink " << name 
                          << ": " << strerror(errno);
            }
        }
    }
    ...
    if (cleaned_count > 0) {
        NEXUS_DEBUG("LargeData") << "Cleanup complete: removed " << cleaned_count 
                  << " channel(s), freed " << (total_freed / 1024 / 1024) << " MB";
    } else {
...
int32_t LargeDataChannel::registerReader() {
    ...
    for (size_t i = 0; i < MAX_READERS; ++i) {
        ...
        if (control_->readers[i].active.compare_exchange_strong(
            ...
            NEXUS_DEBUG("LargeData") << "Registered as reader #" << i 
    ...
    NEXUS_ERROR("LargeData") << "Failed to register reader: max readers (" 
              << MAX_READERS << ") exceeded";
    return -1;
}
...
void LargeDataChannel::unregisterReader(int32_t reader_id) {
    ...
    NEXUS_DEBUG("LargeData") << "Unregistered reader #" << reader_id 
              << ", total readers: " << prev_count << " -> " << (prev_count - 1);
}
