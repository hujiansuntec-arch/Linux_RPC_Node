# Nexus - 高性能进程间通信框架# LibRPC - 高性能进程间通信库# LibRPC - 高性能进程间通信库# LibRpc Communication Framework
...
[![Platform](https://img.shields.io/badge/platform-Linux%20%7C%20QNX-lightgrey.svg)](https://www.qnx.com/)[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)



**Nexus** - 轻量级、高性能的进程间通信框架。专为低延迟和高吞吐量场景设计，支持发布-订阅模式、零拷贝共享内存传输和大数据传输。[![C++14](https://img.shields.io/badge/C%2B%2B-14-blue.svg)](https://en.cppreference.com/w/cpp/14)
...
### 🚀 高性能通信高性能、零拷贝的进程间通信库，专为低延迟和高吞吐量场景设计。支持发布-订阅模式、动态共享内存传输和大数据传输。[![C++14](https://img.shields.io/badge/C%2B%2B-14-blue.svg)](https://en.cppreference.com/w/cpp/14)



- **零拷贝共享内存传输**：V3架构，动态无锁SPSC队列

- **内存优化**：默认33MB/节点（可配置8-132MB）

- **低延迟**：进程内 <1μs，共享内存 <10μs---[![Platform](https://img.shields.io/badge/platform-Linux%20%7C%20QNX-lightgrey.svg)](https://www.qnx.com/)## Features
...
- **自动节点发现**：基于Registry的动态发现### 🚀 高性能通信高性能、零拷贝的进程间通信库，专为低延迟和高吞吐量场景设计。支持发布-订阅模式、共享内存传输和大数据传输。- **Peer-to-peer architecture**: All endpoints are equal nodes
...
- **自动资源管理**：引用计数 + PID检测 + 心跳监控- **零拷贝共享内存传输**：V3架构，动态无锁SPSC队列- **Topic-based pub/sub**: Subscribe to specific topics within message groups

- **异常退出恢复**：崩溃节点2-5秒内自动清理

- **数据完整性**：CRC32校验、序列号检测- **内存优化**：默认33MB/节点（可配置8-132MB）
...
- **QNX平台**：完整支持，条件编译适配- **Triple transport mechanism**: 
...
- [构建系统](#️-构建系统)- **进程内通信**：同进程直接调用（零开销）- **零拷贝共享内存传输**：V3架构，无锁环形队列  - Inter-process: Lock-free shared memory SPSC queues (~500K msg/s)

- [API参考](#-api参考)

- [使用示例](#-使用示例)- **跨进程通信**：动态共享内存（无节点数限制）

- [性能指标](#-性能指标)

- [最佳实践](#-最佳实践)- **自动节点发现**：基于Registry的动态发现- **大数据通道**：专用通道支持高频大数据传输（>1MB）  - Cross-host: UDP broadcast/point-to-point
...
# 方式1: 使用CMake（推荐）- **异常退出恢复**：崩溃节点2-5秒内自动清理
...
- `build/librpc.so.1.0.0` - 共享库（256KB）### 🌐 跨平台支持
...
```cpp- **QNX平台**：完整支持，条件编译适配- **进程内通信**：同进程内零开销- **Lock-free design**: SPSC (Single Producer Single Consumer) queues for shared memory
...
node->publish("sensor", "temperature", 
              ...
              (const uint8_t*)msg.data(), msg.size());- **动态节点发现**：自动注册与发现- **Heartbeat monitoring**: 5-second timeout for zombie node detection
...
```cpp- **Selective delivery**: Only subscribers receive relevant messages
...
node->subscribe("sensor", {"temperature"},- [API参考](#-api参考)

    [](const std::string& group, const std::string& topic,

       const uint8_t* data, size_t size) {- [使用示例](#-使用示例)- **自动资源管理**：引用计数 + PID检测双重清理机制- **Late-joining support**: New nodes automatically discover existing subscriptions

        std::string msg((char*)data, size);

        std::cout << "Received: " << msg << std::endl;- [性能指标](#-性能指标)
...
./build.sh### 📦 易用性### Design Principles
...
# 静态库- **自动初始化**：首次使用时自动清理残留资源1. **Node-centric**: Each endpoint is a Node instance
...
**QNX 平台交叉编译**：make -j4- **类型安全**：C++14强类型接口2. **Topic filtering**: Messages delivered only to matching subscribers
...
export QNX_TARGET=/opt/qnx710/target/qnx7- **零配置启动**：默认参数即可工作3. **Process-aware**: Automatic routing between in-process and inter-process nodes
...
详细说明请参考 [CMAKE_BUILD.md](CMAKE_BUILD.md)- `build/test_*` - 测试程序
...
### Node接口auto node = librpc::createNode("publisher");



```cpp- [快速开始](#-快速开始)Node1 (Process A)           Node2 (Process A)           Node3 (Process B)
...
public:// 发布消息
    ...
    static std::shared_ptr<Node> createNode(std::string msg = "Hello, World!";- [架构设计](#-架构设计)    |                            |                            |

        const std::string& node_id = "",

        const Config& config = Config());node->publish("sensor", "temperature", 

    

    // 发布消息（<256KB）              (const uint8_t*)msg.data(), msg.size());- [性能指标](#-性能指标)    | subscribe("sensor", ["temp"])  subscribe("sensor", ["temp", "pressure"])
    ...
    virtual Error publish(
        ...
        const uint8_t* data,- [API参考](#-api参考)    |                            |                            |
    ...
    **订阅者**：
    ...
    virtual void subscribe(```cpp- [使用示例](#-使用示例)    |                            |                            |

        const std::string& msg_group,

        const std::vector<std::string>& topics,#include "Node.h"
    ...
    virtual std::string getNodeId() const = 0;auto node = librpc::createNode("subscriber");
...
```cppnode->subscribe("sensor", {"temperature"},- [常见问题](#-常见问题)    | (in-process delivery)      |                            |

struct Config {

    // 最大入站队列数（影响内存占用）    [](const std::string& group, const std::string& topic,
    ...
    size_t queue_capacity = 256;     // 默认256，范围: 64-1024        std::string msg((char*)data, size);
...
// 最大: 64 × 1024 × 2048 = 132 MB    |                   (shared memory broadcast)             |
...
// 创建两个节点### 编译库    |                    (Node3 polls SPSC queue)             |
...
// node2订阅    |                            |                  receives message

node2->subscribe("sensor", {"temperature"},

    [](const auto& group, const auto& topic, const auto* data, size_t size) {## 🛠️ 构建系统

        std::cout << "Node2 received: " 

                  << std::string((char*)data, size) << std::endl;```bash```
...
node1->publish("sensor", "temperature", "25.5C");cd librpc
...
nodeA->subscribe("ipc", {"commands"},
    ...
    [](const auto& group, const auto& topic, const auto* data, size_t size) {
        ...
        std::cout << "Received command: " 
                  ...
                  << std::string((char*)data, size) << std::endl;# 清理重建- **Different process, same host**: Lock-free shared memory (Node1 → Node3)
...
while (true) std::this_thread::sleep_for(std::chrono::seconds(1));生成文件：- **Different host**: UDP broadcast (cross-network communication)
...
```cpp./build.sh -d- `lib/librpc.a` - 静态库
...
nodeB->publish("ipc", "commands", "START");# 静态库- `lib/librpc.so` - 动态库## API Reference
...
// ... 填充数据 ...**QNX 平台交叉编译**：### 发布-订阅示例### Node Interface



node->sendLargeData(```bash

    "vision",           // 消息组

    "camera_channel",   // 通道名# 设置QNX环境

    "image_data",       // 主题

    large_data.data(),export QNX_HOST=/opt/qnx710/host/linux/x86_64
...
node->subscribe("vision", {"image_data"},
    ...
    [](const auto& group, const auto& topic, const auto* data, size_t size) {# 交叉编译```cppclass Node {

        // 接收到通知，读取大数据

        auto channel = librpc::LargeDataChannel::create("camera_channel");./build.sh -p qnx
        ...
        if (channel->tryRead(block)) {
            ...
            // 零拷贝访问：block.data 直接指向共享内存

            processImage(block.data, block.header.size);

            channel->releaseBlock(block);详细说明请参考 [CMAKE_BUILD.md](CMAKE_BUILD.md)    using Property = std::string;
...
| 共享内存V3 | 256B | 8μs | 15μs |

| 大数据通道 | 1MB | 35μs | 80μs |

| 大数据通道 | 4MB | 120μs | 250μs |# 清理std::string msg = "Hello, World!";                                       size_t size)>;
...
| 传输类型 | 消息大小 | 吞吐量 | QPS |```node->publish("sensor", "temperature", 
...
| 大数据通道 | 4MB | ~110 MB/s | ~27 |---              (const uint8_t*)msg.data(), msg.size());    // Broadcast message to all subscribers
...
| 配置 | 队列数 | 容量 | 内存占用 | 场景 |## 📚 API参考```    virtual Error broadcast(const Property& msg_group, 
...
| **标准** | 64 | 256 | 33 MB | 高并发 |### Node接口                          const Property& topic, 
...
config.max_inbound_queues = 8;   // 8MB内存    static std::shared_ptr<Node> createNode(#include "Node.h"    // Subscribe to topics

config.queue_capacity = 64;

auto node = librpc::createNode("low_mem", config);        const std::string& node_id = "",



// 高并发场景（服务器）        const Config& config = Config());    virtual Error subscribe(const Property& msg_group, 
...
auto node = librpc::createNode("high_perf", config);    // 发布消息（<256KB）auto node = librpc::createNode("subscriber");                          const std::vector<Property>& topics, 
...
// ✅ 检查返回值        const std::string& topic,

auto err = node->sendLargeData(...);

if (err == librpc::TIMEOUT) {        const uint8_t* data,// 订阅回调

    std::cerr << "Queue full, retry later" << std::endl;

    std::this_thread::sleep_for(std::chrono::milliseconds(10));        size_t size) = 0;

    // 重试...

} else if (err != librpc::NO_ERROR) {    node->subscribe("sensor", {"temperature"},     // Unsubscribe from topics
...
// ✅ 使用RAII自动管理        const std::vector<std::string>& topics,       const uint8_t* data, size_t size) {                            const std::vector<Property>& topics) = 0;

{

    auto node = librpc::createNode("mynode");        DataCallback callback) = 0;
...
// ❌ 避免手动管理    virtual std::string getNodeId() const = 0;        std::cout << "Received: " << msg << std::endl;    // Get large data channel (for high-frequency large data transfer)
...
### 4. 选择传输方式```    });    virtual std::shared_ptr<LargeDataChannel> getLargeDataChannel(
...
| 控制消息 | <1KB | 低频 | `publish()` |### 配置选项        const std::string& channel_name) = 0;
...
2. **异常退出（kill -9）**：下次启动时PID检测自动清理    // 队列容量（每队列消息数）
...
### Q2: 内存占用太大怎么办？// 默认: 32 × 256 × 2048 = 16.8 MB



**A**: 调整配置降低内存占用：// 最大: 64 × 1024 × 2048 = 132 MB### 大数据传输示例// Create a new node
...
librpc::Config config;std::shared_ptr<Node> createNode(const std::string& node_id = "",
...
// 低内存配置：8MB**发送端**：                                 bool use_udp = true,

config.max_inbound_queues = 16;

config.queue_capacity = 64;## 💡 使用示例
...
- ✅ `Node::publish()` - 线程安全// 准备大数据（1MB）

- ✅ `Node::subscribe()` - 线程安全

- ✅ `LargeDataChannel::write()` - 单写者线程安全```cpp

- ✅ `LargeDataChannel::tryRead()` - 单读者线程安全

- ⚠️  不支持多写者或多读者（SPSC设计）#include "Node.h"std::vector<uint8_t> large_data(1024 * 1024);// Get default singleton node
...
**A**: QNX平台差异：// 创建两个节点// ... 填充数据 ...std::shared_ptr<Node> communicationInterface();

- 共享内存路径：`/dev/shmem`（Linux是`/dev/shm`）

- 不支持`MAP_NORESERVE`：立即分配物理内存auto node1 = librpc::createNode("node1");
...
| **[ARCHITECTURE.md](ARCHITECTURE.md)** | 详细架构设计、性能优化、设计权衡 | 高级开发者、架构师 |        std::cout << "Node2 received: " 

| **[CMAKE_BUILD.md](CMAKE_BUILD.md)** | CMake构建系统、交叉编译、IDE集成 | 构建工程师 |

| **[QNX_COMPATIBILITY.md](QNX_COMPATIBILITY.md)** | QNX平台适配、条件编译、差异说明 | QNX开发者 |                  << std::string((char*)data, size) << std::endl;    "vision",              // 消息组
...
2. CMAKE_BUILD.md（构建系统）    "camera_channel",      // 通道名### Example 1: Basic Subscribe and Broadcast

3. ARCHITECTURE.md（深入理解）

4. QNX_COMPATIBILITY.md（平台适配）// node1发布（node2会直接接收，零开销）
...
**Nexus** 意为"连接枢纽"，象征着高效、可靠的通讯中心。作为一个轻量级IPC框架，Nexus致力于：    large_data.data(),    // 数据```cpp



- 🔗 **连接**：无缝连接进程内、跨进程、跨主机通信### 2. 跨进程通信

- ⚡ **高效**：零拷贝、无锁设计、极致性能

- 🛡️ **可靠**：三重资源管理、自动故障恢复    large_data.size()     // 大小#include "Node.h"
...
# 使用便捷脚本nodeA->subscribe("ipc", {"commands"},
...
cd build        std::cout << "Received command: " 

./test_inprocess            # 进程内通信测试

./test_duplex_v2            # 双工通信测试                  << std::string((char*)data, size) << std::endl;**接收端**：
...
- ✅ 跨进程共享内存通信while (true) std::this_thread::sleep_for(std::chrono::seconds(1));// 订阅大数据通知node->subscribe("sensor", {"temperature"}, 
...
- ✅ 资源自动清理node->subscribe("vision", {"image_data"},    [](const auto& group, const auto& topic, const auto* payload, size_t size) {
...
## 📄 许可证```cpp    [](const std::string& group, const std::string& topic,        std::cout << "Temperature: " 



MIT Licenseauto nodeB = librpc::createNode("process_B");
...
## 📈 版本历史// 发送命令（通过共享内存传输到进程A）



### v3.0 (当前) - 内存优化 + 跨平台nodeB->publish("ipc", "commands", "START");        // 接收到通知消息    });
...
### v2.0 - 动态共享内存        // 读取大数据（零拷贝）// Broadcast temperature data
...
- ✅ 心跳监控机制// 发送端        auto channel = librpc::LargeDataChannel::create("camera_channel");node->publish("sensor", "temperature", "25.5C");



### v1.0 - 初始版本std::vector<uint8_t> large_data(1024 * 1024);  // 1MB

- ✅ 基础发布-订阅

- ✅ 进程内/跨进程通信// ... 填充数据 ...        librpc::LargeDataChannel::DataBlock block;```
...
**项目名称**: Nexus - Lightweight High-Performance IPC Framework
